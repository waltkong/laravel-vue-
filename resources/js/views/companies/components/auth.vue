...
<script>
  import {updateCompanyPermissions, companyPermissionOptions} from '@/api/company/company';

  export default {
    name: 'UpdateCompanyPermissions',
    components: { },
    props: {
      createVisible: {
        type: Boolean,
    ...
    created() {
      ...
      if (editRowKeys.length > 0) {
        ...
        this.defaultForm.company_id = this.editRow.id
    ...
    methods: {
      ...
      async storeApi(){
        const response = await updateCompanyPermissions(this.defaultForm);
        if(response.code === 1){
          this.$message({
            message: 'ok',
            type: 'success',
      ...
      async getPermissionOptions(){
        const  response = await companyPermissionOptions({company_id:this.defaultForm.company_id});
        if(response.code.toString() === '1'){
          //permission_options选项 和 has-permissions
          this.permissionOptions = response.data.permission_options;
          this.defaultForm.has_permissions = response.data.has_permissions;
